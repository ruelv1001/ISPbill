...
$dltAllbtn = [];
$tableActions = ['edit' => 'nap.edit', 'delete-item' => 'nap.destroy'];
$addButton = ['Add NAP', 'nap.create'];
...
$customMessage = [
    ...
    'delete' => 'This User will be permanently deleted if you proceed.',
                            ...
                            @endphp
                            <x-table :headers="$headers" :data="$data" title="NAP" :dropdown="$dropdownActions"
                                :actions="$tableActions" tablename="NAP" :addbtn="$addButton"
...
<script>
    ...
    function confirmDelete(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: 'This NAP will be permanently deleted if you proceed.',
            icon: 'warning',
    ...
    function deleteAreaLocation(id) {
        ...
        axios.delete(`/nap/${id}`)
            .then(response => {
                // Show success alert
                Swal.fire('Deleted!', 'The NAP has been deleted.', 'success');
                // Reload or update the page
                location.reload();
            })
            .catch(error => {
                // Handle errors (if any)
                Swal.fire('Error!', 'There was an issue deleting the NAP.', 'error');
            });
    }
