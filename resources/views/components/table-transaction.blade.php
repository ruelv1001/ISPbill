...
@endphp
<div id="{{ $tablename }}-table" style="position: relative; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); border-radius: 0.5rem;">
    <div style="display: flex; align-items: center; justify-content: space-between; background-color: #f3f4f6; padding: 1rem; border-top-left-radius: 0.75rem; border-top-right-radius: 0.75rem;">
        <h3>{{ $title }}</h3>
        @if (!empty($addbtn))
            <a href="{{ route($addbtn[1]) }}" style="background-color: #28a745; color: white; padding: 0.5rem 1rem; border-radius: 0.25rem; text-decoration: none;">
                <x-image src="/images/plus.svg" alt="Placeholder Image" style="width: 20px; height: 20px;" />
                {{ $addbtn[0] }}
            </a>
        @endif
    </div>
    @if ((empty((array) $filters) && count($filters) > 0) || $searchField == true)
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 1rem;">
            <div>
                @if (count($filters) > 0)
                    <form action="{{ isset($filterRoute) ? $filterRoute : route('transaction.index') }}" method="GET"
                        id="filter-form-{{ $tablename }}" style="display: flex; align-items: center; gap: 0.5rem;">
                        <input type="hidden" name="tab-active" value="{{ $tablename }}">
                        <div style="display: flex; align-items: center; gap: 0.5rem;">
                            <x-image src="/images/filter.svg" alt="Placeholder Image" style="width: 18px; height: 18px;" />
                            <div style="display: flex; align-items: center; gap: 0.25rem;">
                                @foreach ($filters as $filter => $values)
                                    ...
                                    @else
                                        <select style="padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.25rem;" name="{{ $filter }}"
                                    ...
                                    @endif
                                @endforeach
                                <button type="submit" style="background-color: #3b82f6; color: white; padding: 0.5rem 1rem; border-radius: 0.25rem;">Apply</button>
                                <a href="/transaction" style="background-color: #ef4444; color: white; padding: 0.5rem 1rem; border-radius: 0.25rem; text-decoration: none;">Clear</a>
                            </div>
                        </div>
                    </form>
                @endif
            ...
            <div>
                @if ($searchField == true)
                    <form action="{{ isset($filterRoute) ? $filterRoute : route('transaction.index') }}" method="GET">
                        <div style="display: flex; align-items: center; background-color: #f9fafb; border: 1px solid #d1d5db; border-radius: 0.5rem;">
                            <input type="hidden" name="tab-active" value="{{ $tablename }}">
                            <input type="search"
                                style="flex: 1; padding: 0.25rem 0.5rem; background-color: transparent; border: none; outline: none;"
    ...
    @endif
    @if ($tableCheckedbox)
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 1rem; display: none;">
            <p><span style="font-weight: bold;">0</span> items selected</p>
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <input type="hidden" name="selected_ids" class="selected-ids">
                @if (!empty($dropdown))
                    <select style="padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.25rem;">
                ...
                @if (!empty($dltAllbtn))
                    ...
                    <button type="button"
                        ...
                        style="background-color: #ef4444; color: white; padding: 0.5rem 1rem; border-radius: 0.25rem;">
                ...
                <button style="background-color: transparent; border: 1px solid #d1d5db; padding: 0.5rem 1rem; border-radius: 0.25rem;">Cancel</button>
            </div>
        </div>
    @endif
    <div style="overflow-x: auto;">
        <table style="width: 100%; text-align: left; color: #6b7280;">
            <thead style="background-color: #f9fafb;">
                <tr style="border-bottom: 1px solid #e5e7eb;">
                    @foreach ($headers as $index => $header)
                        <th scope="col" style="padding: 0.75rem 1.5rem; {{ $index == 'id' ? 'display: flex; align-items: center;' : '' }}">
            ...
            <tbody>
                @if (count($data) > 0)
                        @foreach ($data as $row)
                                    <tr data-trid="{{ $tablename }}-{{ $row['id'] }}" style="border-bottom: 1px solid #e5e7eb;">
                                        @foreach ($headers as $index => $header)
                                                            <td style="padding: 0.75rem 1.5rem; {{ $index == 'id' ? 'display: flex; align-items: center;' : '' }}">
                                                                @if ($index == 'id' && $tableCheckedbox)
                                                                ...
                                                                @else
                                                                                            ...
                                                                                            @if ($index == 'active_due_date')
                                                                                                                        @php
                                                                                                                            ...
                                                                                                                            $colorClass = $dueDate->isPast() ? 'color: red;' : 'color: green;';
                                                                                                                        @endphp
                                                                                                    ...
                                                                                                    @foreach ($actions as $action => $route)
                                                                                                        @if ($action == 'edit' && !$viewOnly)
                                                                                                            <a href="{{ route($route, $row['id']) }}"
                                                                                                                ...
                                                                                                                <x-image src="/images/edit.svg" alt="edit svg" style="width: 20px; height: 20px;" />
                                                                                                            </a>
                                                                                                        @elseif ($action == 'edit-schedule')
                                                                                                            <a href="{{ route($route, $row['id']) }}"
                                                                                                                style="text-decoration: none; color: inherit;">
                                                                                                                <x-image src="/images/view.svg" alt="edit svg" style="width: 20px; height: 20px;" />
                                                                                                            </a>
                                                                                                        @elseif ($action == 'delete')
                                                                                                            ...
                                                                                                            <button type="button"
                                                                                                                ...
                                                                                                                <x-image src="/images/delete-big.svg" alt="delete svg" style="width: 20px; height: 20px;" />
                                                                                                            </button>
                                                                                                        @elseif ($action == 'view')
                                                                                                            <a href="{{ route($route, $row['myid']) }}"
                                                                                                                style="text-decoration: none; color: inherit;">
                                                                                                                <x-image src="/images/view.svg" alt="edit svg" style="width: 20px; height: 20px;" />
                                                                                                            </a>
                                                                                                        ...
                                                                                                        @elseif ($action == 'pay')
                                                                                                            <button type="button"
                                                                                                                ...
                                                                                                                style="background-color: transparent; border: none; cursor: pointer;">
                                                                                                                <x-image src="/images/pay.svg" alt="pay svg" style="width: 20px; height: 20px;" />
                                                                                                            </button>
                                                                                                        @elseif ($action == 'delete-item')
                                                                                                            <button type="button"
                                                                                                                onclick="confirmDelete({{ $row['myid'] }});"
                                                                                                                style="background-color: transparent; border: none; cursor: pointer;">
                                                                                                                <x-image src="/images/trash.svg" alt="pay svg" style="width: 20px; height: 20px;" />
                                                                                                            </button>
                                                                                                        @endif
                                                                                                    @endforeach
                                                                                                </div>
                ...
                @else
                    <tr style="border-bottom: 1px solid #e5e7eb;">
                        <td style="padding: 0.75rem 1.5rem; text-align: center;" colspan="{{ count($headers) }}">No Results Found.</td>
                    </tr>
                @endif
            </tbody>
        </table>
