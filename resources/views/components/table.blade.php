...
<div id="{{ $tablename }}-table" class="table-container relative shadow-md sm:rounded-lg">
    ...
    @foreach ($filters as $filter => $values)
        ...
        @else
            <select class="p-2 table-filter-item filter-item select2-filter" name="{{ $filter }}"
                ...
                <option disabled {{ !request($filter) ? 'selected' : '' }}>{{ ucfirst($filter) }}</option>
    ...
    <button type="button" class="table-filter-btn table-filter-clear" onclick="clearForm(this.form)">Clear</button>
</div>
                        </div>
...
<script>
    ...
    const selectAllCheckbox = document.getElementById('checkbox-all-{{ $tablename }}');
